...
const _ZH_CN_MESSAGES := {
	...
	"Node Name Template": "节点名称模板",
	"BakedZ{z}": "BakedZ{z}",
	"Placeholders: {z}=Sprite2D z_index, {layer}=source layer or parent name, {scene}=scene name, {index}=output number.": "占位符：{z}=Sprite2D 的 z_index，{layer}=源图层名或父节点名，{scene}=场景名，{index}=输出序号。",
	...
	"Bake failed": "烘焙失败",
	"Bake canceled. Existing output was not changed.": "已取消烘焙，已有输出未被修改。",
	...
	"Node name template has an unclosed placeholder.": "节点名称模板中有未闭合的占位符。",
	"Node name template has an unopened placeholder.": "节点名称模板中有未打开的占位符。",
	"Unknown node name placeholder: {%s}. Supported placeholders: {z}, {layer}, {scene}, {index}.": "未知节点名称占位符：{%s}。支持的占位符：{z}、{layer}、{scene}、{index}。",
	"Node name template produced an empty node name.": "节点名称模板生成了空节点名。",
	"Node name template creates duplicate output node: %s. Add {z}, {layer}, or {index}.": "节点名称模板生成了重复节点：%s。请添加 {z}、{layer} 或 {index}。",
	"Target node already exists: %s. Change Node Name Template or enable overwrite.": "目标节点已存在：%s。请修改节点名称模板或启用覆盖。",
	"Replace existing baked output?": "替换已有烘焙输出？",
	"Replace": "替换",
	"Cancel": "取消",
	"The following baked output already exists. Replace it?": "以下烘焙输出已存在。是否替换？",
	"Nodes:": "节点：",
	"Files:": "文件：",
	"not generated by TileMapLayer Baker": "不是由 TileMapLayer Baker 生成",
